...
interface AvatarProps {
  ...
  size?: number;
  /** Shape variant of the avatar (default: "circle") */
...
const STYLES = {
  container: 'inline-block overflow-hidden ring-2 ring-white/10',
  containerCircle: 'rounded-full',
  containerSquare: 'rounded-lg',
...
export default function Avatar({
  ...
  const containerClasses = `${STYLES.container} ${
    variant === 'circle' ? STYLES.containerCircle : STYLES.containerSquare
  } ${className}`;

  return (
    <div className={containerClasses} style={{ width: size, height: size }}>
      <img src={avatarSvg} alt={generateAltText(address)} className={STYLES.image} />
    </div>
  );
}
