...
const STYLES = {
  ...
  chevron:
    "w-3 h-3 text-white/60 transition-all duration-200 group-hover:text-white/80",
...
const WalletDisplay = ({
  address,
  isOpen = false,
}: WalletDisplayProps): React.JSX.Element => (
...
const CopyAddressButton = ({
  onCopy,
  copied,
}: CopyAddressButtonProps): React.JSX.Element => (
  <button onClick={onCopy} className={STYLES.copyButton}>
    ...
    <div className={STYLES.copyContent}>
      <div className={STYLES.copyTitle}>
        {copied ? TEXT.copied : TEXT.copyAddress}
      </div>
